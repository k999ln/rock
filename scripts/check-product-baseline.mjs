...
export function validateBaseline(
  ...
  requireValue(
    data.primaryCapabilities?.includes('autonomous-automation-funds'),
    '動的な自動化ファンド能力が必要です',
  );
  requireValue(
    data.automationFunds?.fundCountLimit === null,
    'ファンド数を固定しないでください',
  );
  requireValue(
    data.automationFunds?.defaultToolCount === 5 &&
      data.automationFunds?.toolCountConfigurable === true,
    '1ファンドは初期推奨5ツールかつ変更可能である必要があります',
  );
  requireValue(
    data.automationFunds?.performanceCommissionBps === 0 &&
      data.automationFunds?.sharedReserveEnabled === false &&
      data.automationFunds?.userOwnsRemainderAfterDirectCostsAndMonthlySkyFee === true,
    '実費と月額上限後の残額は100%利用者へ帰属する必要があります',
  );
  requireValue(
    data.automationFunds?.liveYieldClaimEnabled === false &&
      data.automationFunds?.pooledUserCapitalEnabled === false,
    '未検証利回りと共同資金運用を有効化しないでください',
  );
  requireValue(
  ...
  requireValue(
    data.marketExploration?.appShellAuthorized === true &&
      ...
      data.marketExploration?.strategyLab?.repository ===
        'https://github.com/MrFadiAi/Polymarket-bot' &&
      /^[a-f0-9]{40}$/u.test(
        data.marketExploration?.strategyLab?.reviewedCommit ?? '',
      ) &&
      data.marketExploration?.strategyLab?.mode ===
        'pinned_clean_tree_offline_backtest_only' &&
      data.marketExploration?.strategyLab?.privateKeyAccepted === false &&
      data.marketExploration?.strategyLab?.liveModeToggleEnabled === false &&
      data.marketExploration?.strategyLab?.countsAsFundRevenue === false &&
