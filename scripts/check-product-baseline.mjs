...
export function validateBaseline(
  ...
  requireValue(
    ...
    '確定要望RQ01〜RQ19の順序/欠落/重複を確認してください',
  );
  ...
  const passport = data.sky?.executionPassport;
  requireValue(
    passport?.separateDeclaredFromObserved === true,
    '実行パスポートは開発者申告と観測済み接続を分けてください',
  );
  requireValue(
    passport?.preferredArchitecture ===
      'rockstaros_hardware_local_compute_storage_control',
    'RockstarOS端末内の計算・保存・制御を主経路として保持してください',
  );
  requireValue(
    passport?.selfGenerationStatus === 'not_implemented_not_verified',
    '自家発電の未実装・未検証境界を保持してください',
  );
  for (const field of [
    'primaryTarget',
    'supportedTargets',
    ...
    requireValue(
      passport?.requiredFields?.includes(field),
      `実行パスポートに${field}が必要です`,
    );
  requireValue(
...
if (
  ...
  console.log(
    '製品ベース: RQ01〜RQ19、実行パスポート、ATM手数料0、ATM独立、1.0構成、導入計画、受入雛形、入口、監査SHA、作成規約を確認（意味の一致と最新進捗は別途レビュー）',
  );
}
