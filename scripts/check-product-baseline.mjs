...
export function validateBaseline(data, read = (path) => readFileSync(path, 'utf8')) {
  ...
  const expected = Array.from({ length: 19 }, (_, i) => `RQ${String(i + 1).padStart(2, '0')}`);
  requireValue(JSON.stringify(data.requirements) === JSON.stringify(expected), '確定要望RQ01〜RQ19の順序/欠落/重複を確認してください');
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  console.log('製品ベース: RQ01〜RQ19、ATM手数料0、ATM独立、1.0構成、導入計画、受入雛形、入口、監査SHA、作成規約を確認（意味の一致と最新進捗は別途レビュー）');
}
