...
export function validateBaseline(data, read = (path) => readFileSync(path, 'utf8')) {
  ...
  const expected = Array.from({ length: 20 }, (_, i) => `RQ${String(i + 1).padStart(2, '0')}`);
  requireValue(JSON.stringify(data.requirements) === JSON.stringify(expected), '確定要望RQ01〜RQ20の順序/欠落/重複を確認してください');
  ...
  requireValue(data.primaryCapabilities?.includes('goal-driven-brand-operations'), '目標駆動のブランド運営能力が必要です');
  requireValue(data.fashionBrandOperations?.externalEffectsExecutedByAutopilot === false,
  ...
  requireValue(data.marketExploration?.runtimeAuthorized === true && data.marketExploration?.realValueEnabled === false,
    '予測市場runtimeはSIMULATION/PAPERのみ承認済み、LIVE実資金は禁止です');
  requireValue(JSON.stringify(data.marketExploration?.allowedModes) === JSON.stringify(['SIMULATION', 'PAPER']),
    '予測市場の標準モードはSIMULATION/PAPERのみです');
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  console.log('製品ベース: RQ01〜RQ20、Value/SpendはSIMULATION/PAPERのみ、ATM手数料0、ATM独立、1.0構成、導入計画、受入雛形、入口、監査SHA、作成規約を確認（意味の一致と最新進捗は別途レビュー）');
}
