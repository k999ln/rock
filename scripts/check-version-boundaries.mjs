...
export function validateVersionBoundaries({
  ...
  assert.equal(boundaries.product.name, productIdentity.displayName);
  assert.equal(boundaries.product.releaseState, 'developer_preview_not_public');
  assert.equal(
    boundaries.product.displayVersion,
    `${productIdentity.version} ${productIdentity.releaseStageLabel}`,
  );
  assert.equal(
    productIdentity.previewName,
    `${boundaries.product.name} ${boundaries.product.displayVersion}`,
  );
  assert.ok(
    previewNotes.includes(productIdentity.previewName),
    'preview release notes must display the canonical product version',
  );
  for (const token of ['PRODUCT_NAME', 'PRODUCT_VERSION', 'PRODUCT_PREVIEW_NAME']) {
    assert.ok(
      guidePage.includes(token),
      `preview guide must render canonical identity token ${token}`,
    );
  }
  ...
  return {
    ...
    productVersion: productIdentity.previewName,
...
export function validateRepositoryVersionBoundaries() {
  ...
  return validateVersionBoundaries({
    boundaries,
    productIdentity: readJson('data/product-identity.json'),
