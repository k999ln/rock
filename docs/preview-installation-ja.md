...
Hub で「引用整理」を開き、用途・作者・版・権利・実行先・費用と許可を確認します。「v1.0.0 をインストール」→「この権限を確認して利用を許可」→「ツールを開く」と進みます。「サンプルを入力」を押し、入力を確認して「実行する」を押します。画面上のサンプルは次の同梱ファイルと一致します。

`native/os/tools/fixtures/citations.md`:
...
結果画面で本文中の出典が `[1]` になり、末尾の出典一覧に「店舗情報」が表示され、コード内の例がそのまま残ることを確認します。「履歴」から完了した同じ処理を開き、保存結果を再表示します。引用整理は入力の整理です。記事生成、出典の真偽確認、自動納品、売上発生とは扱いません。Wallet で見積・予約・確定費用・保留・入金の区別と「合成」「未接続」の表示を確認してください。実行成功から実売上や実資金を作りません。
