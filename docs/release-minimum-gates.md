...
| QEMU Developer Preview配布 | BLOCKED | 2/5 | 製品ライセンス、正式署名、同一最終候補のfresh導入・更新・復旧 |
...
- `npm run release:sbom` でCycloneDX 1.6のWeb/npm SBOMをignored `work/release/`へ生成する。現在は854 unique componentで、native Buildroot inventoryとは分離する。
