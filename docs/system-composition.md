...
現在選んでいる組合せは1.0の目的に整合する設計候補である。最適性や全経路の実装完了は未実証。詳細な層別責任、model/記憶/仕事/能力交渉、Coreと応用の受入は[AIネイティブOS共通設計](ai-native-os-architecture.md)を参照する。Material Invention／avocadoMiniの全体像と参加入口は[共有用完成設計書](rockstaros-avocado-mini-complete-design.md)を正本とする。

製品の中心は、交換可能な高性能ローカルLLMとoffline agent runtimeを持つ **AIネイティブOS Core** である。SkyとZemaはそのCoreを最初に実用化する第一者system、仕事・生活を便利にする自動化は継続的な製品開発系統、Material Invention／avocadoMiniは物質digital twinから発明候補を作る主要応用system、ゲーム・IP／動画は関心に基づく優先的な応用系統とする。個別systemをOS imageへ密結合せず、共通の権限、仕事、Tool、receipt、更新・復旧契約で接続する。
...
- Material Invention Coreを物質・工程・安全・証拠の正本、avocadoMiniをVR／AR／2Dの標準操作面とする
- Tool完了、検証済み収益、Wallet反映、ファンド実績を別状態で管理
- 運営管理画面はOS外、ゲームは関心に基づく優先的な応用系統。ただし1.0のCore実証を止める必須依存にはしない

ただし、**全体が一つの製品として稼働済みという意味ではない**。Material Inventionは装置非接続sandbox Coreと統合設計までで、XR runtime、四方向sensor実機、simulation／Patent AI Provider、外部ラボ接続は未実装である。native Sky選択のBroker永続化、Zema／Local AI API v2／固定Tool、合成Wallet、Shell、Operator検査、実再起動2段階は[所有GL066の試験署名APK 23項目](evidence/android-pixel-10-prefull-physical-20260916.json)で合格した。backupは非破壊exportまで。model/runtimeの汎用交換、長期記憶、外部変更Toolのoutbox、多端末Sky、GameのAndroid接続は未実装。鍵喪失復元、外部Provider、AOSP imageは未受入。本番準備完了、収益や利回り、実材料性能、特許性、実機OS完成は主張しない。
...
| Material Invention／avocadoMini | Coreで物質・工程・安全・証拠を管理し、四方向sensorとVR／AR／2Dで操作 | 適合 | 統合設計とsandbox Coreは完成。XR runtime、四方向実機、simulation／Patent AI接続は未実装 |
...
Material Inventionについては、`物質digital twin → 手操作 → 安全gate → 差分simulation → 発明履歴 → Patent AI／外部ラボ`のうち、Core contract、合成fixture、候補graph、fail-closed安全判定、共有用統合設計までがある。Core graphからの決定的scene生成、合成pose操作、四方向校正、実simulation Provider、Patent AI provenance bridge、資格を持つ外部ラボ受入が未接続である。これらは段階ごとに別の合格証拠を必要とする。

## 進める順番

1. 完了済みの固定Tool実再起動を回帰基準にし、model互換/最小記憶/effect分類/能力交渉をapp-onlyで具体化する。便利Tool、非金融Game/IP、MAT05の合成scene／pose受入を並行して進める。
