...
同日のSkyフロント改善を反映し、商品一覧型の旧画面から、依頼受付・役割チップ・ダークなTimelineを持つ役割フィードへ移行した。`ブランド運営役`とInstagram／広告／DM／受注／決済／制作／発送の語から本商品へ決定的に案内し、商品ダイアログでは既存の38 MCP操作画面をそのまま利用する。サブスク顧問と既存4役も同じ画面に残している。
