...
def main():
    ...
    update_boundary_path = Path('/var/tmp/rock-final-c-update-closed-boundary.json')
    assert hashlib.sha256(update_boundary_path.read_bytes()).hexdigest() == '2749146b9a83b35f019fd17f2e5deaa22b9cc82fb9393e3260fc05356e7246fc'
    update_boundary = json.loads(update_boundary_path.read_bytes())
    assert update_boundary['status'] == 'PASS_STOPPED_BOUNDARY_ORIGINAL_FAILURES_RETAINED'
    assert update_boundary['guest_running'] is False and update_boundary['authority_running'] is False
    assert update_boundary['new_job_purchase_charge_or_ui_inputs'] == 0
    assert update_boundary['remote_requests_submitted_in_failed_citation'] == 0
    assert update_boundary['root_c_config_sha256'] == hashlib.sha256(args.config.read_bytes()).hexdigest()
    assert hashlib.sha256(Path('/var/tmp/rock-final-c-9abf78a-continuation-02/orchestration.json').read_bytes()).hexdigest() == update_boundary['original_ui_failure_orchestration_sha256']
    equivocation_path=Path('/var/tmp/rock-final-c-equivocation-closed-boundary.json')
    assert hashlib.sha256(equivocation_path.read_bytes()).hexdigest() == '06c242ef8314090fc83834c1540c65ba909e1a848a171d7803e7412d8bfdff69'
    equivocation=json.loads(equivocation_path.read_bytes())
    assert equivocation['status']=='PASS_STOPPED_BOUNDARY_ORIGINAL_FAILURES_RETAINED'
    assert equivocation['guest_running'] is False and equivocation['authority_running'] is False
    assert equivocation['remote_requests_submitted_in_failed_citation']==0
    assert equivocation['root_c_config_sha256']==hashlib.sha256(args.config.read_bytes()).hexdigest()
    assert hashlib.sha256(Path('/var/tmp/rock-final-c-9abf78a-continuation-03/orchestration.json').read_bytes()).hexdigest()==equivocation['original_ui_failure_orchestration_sha256']
    ...
    for name, script in names:
        path = Path('/var/tmp/rock-c-citation-selector-1213/systems/rock-star-os/os/desktop/observe-pc-link-os.py') if name == 'citations' else source / 'os/desktop' / script
        if name == 'citations': assert hashlib.sha256(path.read_bytes()).hexdigest() == '58edfb30d45cde34367db2ba8e05b79d787e4040f9cd6c7c4f375dbaa06223df'
        ...
        if name == 'citations':
            command += ['--fixture', 'citations', '--existing-local-citation', '--service-state', '/var/tmp/rock-final-c-9abf78a-continuation-02/citations']
        stages.append({'stage': name, 'observer_sha256': hashlib.sha256(path.read_bytes()).hexdigest(),
                       'command': command, 'status': 'NOT_RUN'})
    report = {'schema': 'rock-final-user-flow-continuation/3', 'equivocation_boundary_sha256': hashlib.sha256(equivocation_path.read_bytes()).hexdigest(), 'update_boundary_sha256': hashlib.sha256(update_boundary_path.read_bytes()).hexdigest(), 'citation_observer_commit': '664f67a5e979299b21b1ca801e8ea771b20ca4ce', 'process_umask': '0077', 'previous_orchestration_sha256': hashlib.sha256(previous_path.read_bytes()).hexdigest(), 'preflight_correction_sha256': hashlib.sha256(boundary_path.read_bytes()).hexdigest(), 'reused_complete_stages': previous['stages'][:3], 'source_commit': args.commit,
              ...
              'scope': 'Existing closed dedicated device after original Game/audit/financial PASS and zero-guest citation preflight FAIL. Only original remaining citations/demo probes; previous whole FAIL retained. Corrected host process umask and explicit stopped-baseline citation update selector, reuse original guest-pinned signed registry state (no new revision1 or pin reset), no repeated credit/purchase/financial action. Original failed boots normally closed with separate proofs.'}
