...
def main():
    ...
    def demo_capture(label):
        ...
        captured = subprocess.run(command, cwd=source, env=env, capture_output=True, timeout=30)
    ...
    try:
        for stage in stages:
            ...
            if stage['stage'] == 'demo':
                ...
                compared = subprocess.run(['/usr/bin/python3', '-B', str(source/'os/desktop/game_authority_ui_retention.py'),
                    'compare', '--policy', 'index-identity-maximum-time-only/1', '--before', str(demo_before), '--after', str(demo_after)],
                    cwd=source, env=env, capture_output=True, timeout=30)
