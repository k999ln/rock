...
base=Path('/var/tmp/rock-final-9abf78a/systems/rock-star-os');closed=Path('/var/tmp/rock-final-c-update-failure-normal-close-01');failed=Path('/var/tmp/rock-final-c-9abf78a-continuation-02');target=Path('/var/tmp/rock-final-c-update-closed-boundary.json');assert not target.exists()
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
r=json.loads((closed/'report.json').read_bytes());assert r['status']=='FAIL' and r['error']=="OSError(98, 'Address already in use')" and r['owned_device_running'] is False
