#!/usr/bin/env python3
import hashlib,importlib.util,json,os,select,signal,socket,sys,time,traceback,subprocess
from datetime import datetime,timezone
from pathlib import Path
os.umask(0o077);os.environ['PATH']='/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin'
base=Path('/var/tmp/rock-final-9abf78a/systems/rock-star-os');failed=Path('/var/tmp/rock-final-c-9abf78a-continuation-03/citations');out=Path('/var/tmp/rock-final-c-equivocation-recovery-normal-close-02');out.mkdir(mode=0o700,exist_ok=False)
...
config=json.loads(Path('/var/tmp/rock-release-os-20260910/final-9abf78a-empty-authority/root-c-device.json').read_bytes());prior=json.loads((failed/'1/report.json').read_bytes());summary=json.loads((failed/'summary.json').read_bytes());record=json.loads((failed/'1/owned-record.json').read_bytes())
...
try:
 ...
 for service in summary['services']:
  ...
  try:
   ...
   fields=(proc/'stat').read_text().rsplit(')',1)[1].split();ticks=int(fields[19]);btime=next(int(l.split()[1]) for l in Path('/proc/stat').read_text().splitlines() if l.startswith('btime '));assert abs(btime+ticks/os.sysconf('SC_CLK_TCK')-service['started_unix'])<2
 ...
 original=Path('/var/tmp/rock-final-c-9abf78a-continuation-02/citations')
 old_summary=json.loads((original/'summary.json').read_bytes())
 old_registry=next(x for x in old_summary['services'] if x['label']=='registry')
 assert str(original/'registry') in old_registry['argv']
 log=(out/'reused-registry.log').open('xb');registry=subprocess.Popen(old_registry['argv'],cwd=base,stdout=log,stderr=subprocess.STDOUT,env=dict(os.environ,PYTHONPATH=str(base/'os')+':'+str(base/'src')))
 b.guest.save(out/'reused-registry.json',{'pid':registry.pid,'argv':old_registry['argv'],'original_state':str(original/'registry'),'original_summary_sha256':hashlib.sha256((original/'summary.json').read_bytes()).hexdigest()})
 from registry.transport import HTTPSOrigin
 origin=HTTPSOrigin('https://127.0.0.1:9443',base/'os/registry/fixtures/development-ca.pem')
 deadline=time.monotonic()+10
 while True:
  assert registry.poll() is None
  try:
   with socket.create_connection(('127.0.0.1',9443),timeout=.5):break
  except OSError:
 ...
 driver.native.capture('before-original-catalog-refresh')
 # An explicitly diagnosed recovery action at the visible enabled button in
 # the retained 720x960 frame; original failing probe remains FAIL. This is
 # not a relaxed success observer or a bypass of registry verification.
 driver.native.click(608,130);time.sleep(1)
 driver.wait('署名確認済み',label='original-signed-index-restored')
 ...
 registry.terminate();registry.wait(timeout=10);assert registry.returncode==-signal.SIGTERM;log.close();report['reused_registry_exit_code']=registry.returncode
