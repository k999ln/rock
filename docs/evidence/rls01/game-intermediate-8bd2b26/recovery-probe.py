...
root=Path('/Users/kaiya/Library/RockstarOS/rls01-game1');env=dict(os.environ,LIMA_HOME=str(root/'lima'));base=['limactl','shell','--workdir','/','os']
pre=json.loads(Path('work/game1-pending-guards.json').read_text())['state'];intent=pre['gate']['intent'];record=json.loads((root/'installation.json').read_text())
...
Path('work/game1-recovery.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({k:v for k,v in report.items() if k!='state'}))
