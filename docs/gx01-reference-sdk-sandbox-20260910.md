...
This is a public synthetic integration fixture. It has no real value, external game or commercial rate. The reference owner/author clients, owned host lifecycle, actual process-kill boundaries and issuer epoch current-copy recovery are implemented and have scoped host evidence. Final OS-image and fresh-install acceptance remain separate gates.
...
The device facade shares the existing remote Wallet contract and cache. It adds a dedicated private Game journal beneath `/data/wallet/game-client`; it does not create another cash ledger. Its health response and Hub service initialization do not require a running Game authority. Fresh TLS status determines connected state. A new UI begin click after restart recovers a still-live persisted pending connection intent. If that intent has expired, or current owner TLS reports an approved connection EXPIRED or REVOKED, a new key is refused with an explicit explanation that reconnection is unsupported in this version. The original begin key and the narrow connection client’s explicit retry remain exact receipt recovery; they never extend the approval or connection lifetime. Pending consent expiry also appears as EXPIRED in the catalog. The permanent game/player owner reservation, existing receipts, purchase history and reconciliation remain unchanged.
...
`snapshot` refuses a running sandbox and holds the actual C, router, Wallet and all Game locks. It reads all five DBs, unknown SQLite tables/rows/schema and retained JSON identities without constructing services or recovering journals. Financial/authentication/account tables are initially empty. It is an observation command, not a backup/restore claim. The implemented stopped current-copy path is described in `gx01-current-copy-sdk-recovery-20260910.md`; device-only exports cannot recover these external authorities. Initial snapshots contain five DBs. After current-copy, retained original Wallet/Entitlement DBs are also included and fenced, so the inventory grows rather than hiding retired source data.
