...
Skyはアプリ一覧ではなく、仕事を受け付ける窓口にする。利用者は「この記事の出典をまとめて」のように依頼し、Skyが実行可能な役へ振り分ける。現在は次の4役を固定規則で選び、知らない依頼を勝手に実行しない。

```text
利用者の依頼
    │
    ▼
Sky受付 ── 判断できない ──> 4役から本人が選択
    │
...
現在の実装はLLMエージェントではなく、説明可能なキーワード振り分けである。Grokのように幅広く会話し、複数手順を計画・再計画する機能は未実装。今の4役が確実に動くことを優先し、LLMを追加する場合も、実行権限・送信先・料金の確定は決定的なPolicy Brokerへ残す。
