...
Schema creation and migration run transactionally. Schema v2 migrates v1 approval rows into the two-stage approval model and stops legacy unconfirmed grants, while an unknown newer version fails closed without reset. Legacy backups are owner-scoped binary snapshots encrypted with AES-256-GCM under a non-exportable Android Keystore key. `avocadoos-recoverable-backup/2` uses a fresh AES-256-GCM data key for every backup and wraps it through both a separate device Keystore key and an independent 256-bit owner recovery secret represented by a checksum-protected 24-word phrase. The Shell shows the phrase locally and activates it only after four requested words match; this phrase is not a Wallet seed. The active Binder path exports v2 and imports only into an empty owner target, creates a fresh device binding, pauses automation, rotates the Sky token, stops active approvals and omits installed-component authority. Application backup remains disabled. Source, Java and Android-emulator acceptance passed; the physical wipe/restore drill remains mandatory. The complete boundary is [Android backup recovery](android-backup-recovery.md).
...
Host unit tests cover registry isolation, approval scope and lifecycle, cost caps, ledger idempotency and reversals, update/rollback checks, schema failure, legacy authenticated encryption, both v2 unwrap routes, strict recovery-phrase decoding, transactional restore, Engine request lookup, native Sky persistence and strict Zema plan parsing. Android Gradle compiles Broker, Shell and Tool; emulator tests prove that a no-INTERNET Shell with only `USE_SHELL_API` can persist one selection, use the v2 recovery flow, reject an unpersisted token, submit and read progress through the signed Broker while lacking `MANAGE_PLATFORM`. The current source and emulator run are not evidence of whole-flow physical reboot recovery, a physical Keystore-loss wipe restore, AOSP image build, SELinux enforcing boot, production signing, OTA rollback, or full physical-device acceptance; those remain release gates.
