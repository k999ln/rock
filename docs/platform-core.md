...
Host unit tests cover registry isolation, approval scope and lifecycle, cost caps, ledger idempotency and reversals, update/rollback checks, schema failure, legacy authenticated encryption, and both v2 unwrap routes with negative owner/key/tamper cases. Android Gradle compiles and lints Broker, Shell and Tool; the emulator test proves that a no-INTERNET Shell with only `USE_SHELL_API` can submit and read progress through the signed Broker while lacking `MANAGE_PLATFORM`. The current source is not evidence of a successful AOSP image build, Keystore-loss data import, SELinux enforcing boot, production signing, OTA rollback, or physical-device acceptance; those remain release gates.
