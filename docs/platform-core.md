...
Shell API v2 adds a native Zema request without giving the UI, model or Tool direct access to Broker storage. The owner-selected Tool ID and natural-language request enter `ZemaOrchestrator`; Local AI Binder API v2 exposes a separate plan-only call for the exact `article-preparation@1/input-v1` schema. The runtime uses JSON Schema constrained decoding with Tool calls disabled. `ZemaToolPlan` then independently requires one exact JSON object, rejects wrappers, unknown fields and Tool substitution, and proves both pure Tool transforms can accept the closed input before `Engine.submit` creates any work. Request IDs are idempotent. A model proposal, invalid plan, missing model or denied consent returns a bounded structured `blocked` response and creates no partial work.

This first connection is deliberately limited to `article-preparation@1`; it is not yet the general Sky Tool registry. Android 15 emulator passed 8 Broker and 3 Shell tests, including a no-model stop with zero partial work. Stock Pixel 10 passed the same counts: the Shell path queued a validated Zema plan, and the Broker integration ran `citations@1` then `free-article@1`, stored a readable result and five history events, and ended in human `review`. Native Sky handoff, whole-flow reboot/failure recovery and AOSP integration remain open. See [plan v2 physical evidence](evidence/android-local-ai-plan-v2-20260916.json); the earlier rejected-plan record remains as fail-closed regression evidence in [the initial Zema record](evidence/android-zema-selected-tool-20260916.json).
...
Host unit tests cover registry isolation, approval scope and lifecycle, cost caps, ledger idempotency and reversals, update/rollback checks, schema failure, legacy authenticated encryption, both v2 unwrap routes, Engine request lookup, and strict Zema plan parsing with negative owner/key/tamper/Tool-substitution and non-executable-input cases. Android Gradle compiles Broker, Shell and Tool; device tests prove that a no-INTERNET Shell with only `USE_SHELL_API` can submit and read progress through the signed Broker while lacking `MANAGE_PLATFORM`. They also prove both zero-partial-work failure without a model and one successful physical plan-to-result path. The current source is not evidence of native Sky persistence, whole-flow physical reboot recovery, AOSP image build, Keystore-loss data import, SELinux enforcing boot, production signing, OTA rollback, or full physical-device acceptance; those remain release gates.
