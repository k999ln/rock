...
**判定: INCOMPLETE。D0〜D5は下記のQEMU開発条件でPASS、D6はFAIL。** 2026-09-09のrun42は、正常終了を伴う4サイクルと反復5件の記録後、6件目の結果認識が事前のUI期限30秒を超えて停止した。60分・61件の受入には到達していない。保存画面で結果が見えることを、期限付き試験の成功へ置き換えない。
...
機械記録は [受入JSON](evidence/os-base/acceptance-b8287bc-20260909.json)、照合結果と入力hash一覧は [独立監査JSON](evidence/os-base/acceptance-audit-b8287bc.json)。監査は101ファイルのsnapshot、凍結commitの778ファイル、業務・復元の実PNG179枚、報告間の参照hashとイメージ識別値を照合した。16件の照合が通ったことは、D6の失敗を解消しない。
...
| 後続GX00 host実装 | `023976d6d5949b1fbce903e3faa89fc27a1fd18d`。上記OSには含まれない |
...
| D6 | 事前計画は5正常サイクル・61 jobs・3600〜4200秒。run42は4正常終了後、反復5件を記録。6件目の結果OCRで元の30秒期限を超過しFAIL。残り時間・資源・終了/保存の全条件は未達 | **FAIL**。[固定計画](evidence/os-base/soak-plan42-b8287bc.json)、[失敗raw報告](evidence/os-base/soak42-ocr-failed-b8287bc.json)、[原log](evidence/os-base/soak42-ocr-failed-b8287bc.log) |
...
2回目の終了監視は操作待ちが300秒を超えFAILを保存。その後の実画面による通常終了を、同じsessionの停止後検査で確認した。電源以外43表/schema/sequenceと全3receipt/1jobを保持し、電源1件だけ追加。元FAILと未観測QMP eventは埋めず、D6の継続稼働は独立した43で判定する。
...
| 残項目 | 今回の判定・次の条件 |
| --- | --- |
| D6 | 42の保存画面と停止後台帳を確認済み。host OCR修正はMac/Linux各68件、実C描画66例で同じconfidence45のまま通過。[修正記録](evidence/os-base/host-result-ocr.json)。新規43で全5サイクル・60分・61件を同じ期限/資源上限で再試験中 |
| GX0/GX1/GX2/GX3/DX | 凍結OSではNOT_RUN。023976dのhost実装や接続schema検証を動作するゲームendpoint/SDKとは呼ばない。逆方向は既定無効 |
...
6. run42失敗と追加調査を保持し、新しい計画・同じ閾値でD6を再試験する。run43はbase host commitにOCR 6ファイルのみを加えたpayload `10961504c950726c2f445fca966b80f6bab32772d037b58b1f0ea153403367a8`を使用し、573ファイルと実rootfsの6 source bindingを照合済み。計画、全sample、画面、失敗、各closed DB/receipt、終了状態を回収してからこの報告を更新する。全必須条件が通るまで「QEMU OS雛形合格」としない。
