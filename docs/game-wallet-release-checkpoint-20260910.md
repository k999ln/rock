...
- RQ13/GX00/GX01/DX01 / 期限・永続所有者予約と元要求回収を維持 / 期限切れintentを新クリックで再表示する不便 / 既存strict TLS status・単調時計・exact retryを再利用 / 新keyだけ説明付き再接続未対応拒否、未同意期限到達もcatalog EXPIRED / 実TLSのpending expiry・active expiry・revoke後に元receipt/残高/完了購入照合保持、既存初回/再起動sampleを含む13tests17.591s skip0 PASS / `work/gx01-expired-connection-fixed-test-linux.log` SHA256 0f60177bf41db34722650e72535a3cbf31f246afcd1ef39f656cfe1de2f3d4cf。原新testのwrong retry API呼出3ERRORは `work/gx01-expired-connection-linux.log` SHA256 b4fd5e4642aa826f15b8e14c1c10d6076a81e8eaa5d459f2a1658290589b4d7fに保持。DB/schema/所有者移転/TTL変更なし、実OS表示と最終source fresh SDKは別受入。

- GX00/GX01/DX01 / 最終sourceと実測範囲を固定 / 中間SHAの合格へ読み替える不便 / 既存実TLS/typed保持/旧client/C/SIGKILL/Node契約を再利用 / 最終9abf78a原692filesで24要件を事前plan固定 / 原188runは177PASS＋保存harness0775拒否11件、修正0700の11実SIGKILLは44.072s全PASS、Node6PASS / `docs/evidence/gx01/final-9abf78a-20260910.json`。原188全PASSとは合算表示せず、原arm64 full10TLSdeadline ERRORと原因未確定も別保持。追加runtime/期限/guard変更なし、最終OS/fresh SDKは他gate。
