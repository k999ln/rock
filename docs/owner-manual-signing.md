...
同日、内蔵FileVault領域に本人専用mode0700のapplication-data directoryを作成し、`storage-check`の実readbackが`ENCRYPTED_OWNER_PRIVATE_STORAGE_READY`で合格した。ローカル絶対pathとUIDはrepositoryへ記録しない。これは空directoryの準備だけで、production鍵は0件、別の暗号化backupも未準備。OWNER_MANUALの採用、鍵生成、公開trust、失効手順、実署名を承認した証拠にはしない。
