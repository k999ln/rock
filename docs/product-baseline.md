...
版: 1.1 / 決定日: 2026-09-09 / 正本: `k999ln/rock`。

この文書は利用者がこの日に明示した製品要望を固定する。実装状況は [追補の進捗監査](progress-audit-20260909-followup.md)（[初回監査](progress-audit-20260909.md)は履歴）、毎回の確認方法は [プロンプト作成規約](prompt-playbook.md) を参照する。決定と実装実績を同じものとして扱わない。
