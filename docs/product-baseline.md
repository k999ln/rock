...
2026-09-12追記（v1.14）: Chatの既定を **Sky Auto** とし、利用者が先にアプリを選ばず自然文で依頼できるようにする。Sky Autoは接続済みアプリの役割と依頼内容を照合し、選んだ担当を会話内へ表示する。アプリの直接指定は任意の切替として残す。実行receiptは会話より目立たせず、最近の処理としてまとめる。判別不能、未接続、追加情報が必要な場合は推測実行せず、その理由と次の一手をChatで返す。
