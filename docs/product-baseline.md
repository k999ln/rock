...
2026-09-21 avokado Proの新指定: 利用者は、avocadoMiniの4本のMotion Towerの奥にある低いボックスを起点に、Mac miniのように置ける高性能サーバー **avokado Pro** を開発すると指定した。希望参考価格は88万円。RockstarOS、ローカルLLM、大容量storage、独立GPUを持ち、avocadoMiniの演算coreとしても単体computerとしても使う。GTA6をプレイできる製品を開発目標にする。容量だけではgame対応を確定できないため、Rockstarとの合意対象、許諾build／実行方式、GPU driver、認証、controller、描画、save／updateを実機で受け入れた後に動作対応を表示する。P0.1設計は `docs/avokado-pro-hardware-design.md`、公開構想ページは `/pro/` を正本とする。実機、RockstarOS x86-64製品image、GTA6動作、税・送料を含む販売価格は未完成。
