2026-09-20 予約販売価格の新指定: 利用者はMotion Tower 1本を16万円、4本を41万円、いずれも税別と指定した。4本キットには従来の構想どおりEdge Hub 1台を含む案とする。旧「41万円税込の目標価格」は価格表示の現行指定として撤回する。発送時期は未定、Instagram `@kirin.41` は連絡先リンクとして使用する。送料、販売者の所在地・電話番号、キャンセル・返金条件、決済事業者の本番接続は未確定であり、これらが揃うまで全額決済を開始しない。GTA 6の対応実績はなく、製品互換性を表示しない。
