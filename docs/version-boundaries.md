...
RockstarOSの版表示は一つの数字ではない。製品表示、Web package、native開発基盤、QEMU配布候補、Android試作を別々の責任境界として扱う。正本は `data/version-boundaries.json`、自動検査は `npm run version:check` とする。

| 境界 | 現在値 | 意味 |
| --- | --- | --- |
| 製品表示 | 1.0 Developer Preview | 利用者向けの開発版名称。一般公開済みを意味しない |
...
| Android試作 | versionName 0.1.0 / versionCode 1 | source・emulator試作のAPK識別。実機公開版ではない |
