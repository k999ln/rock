...
WORK=ROOT/'work/rocketstar_c3'
PACK=ROOT/'outputs/rocketstar_C3_package'
PACK.mkdir(exist_ok=True)
STEM='rocketstar_C3_Integrated_Design_Baseline'
...
class C3Doc(DesignDoc):
    def __init__(self,path):
        super().__init__(path)
        self.title='rocketstar C3 統合設計基準書'
        self.author='rocketstar Design Study'
    def footer(self,c,doc):
        ...
        c.drawString(LEFT,21,'rocketstar  /  C3    |    外観・方式の基準案    |    製造・飛行 未承認')
...
def rocket_layout():
    ...
    x=88;y=24;w=58
    d.add(Rect(x,y,w,162,fillColor=colors.HexColor('#B8C0C5'),strokeColor=NAVY))
    d.add(Rect(x,186,w,10,fillColor=NAVY,strokeColor=NAVY))
    d.add(Rect(x,196,w,80,fillColor=colors.HexColor('#B8C0C5'),strokeColor=NAVY))
    d.add(Polygon([x,276,x+10,293,x+29,310,x+48,293,x+58,276],fillColor=colors.HexColor('#B8C0C5'),strokeColor=NAVY))
    # Functional regions only; not tanks, bulkheads or structural stations.
    label(d,x+w/2,220,'上段',11);label(d,x+w/2,99,'第1段',11)
...
def satellite_layout():
    ...
    for x in [20,371]:
        ...
        for q in range(1,6):d.add(Line(x+q*110/6,130,x+q*110/6,193,strokeColor=colors.HexColor('#86ABC3'),strokeWidth=.5))
        d.add(Line(x,161,x+110,161,strokeColor=colors.HexColor('#86ABC3'),strokeWidth=.5))
...
assets={
 '{{ROCKET_LAYOUT}}':(rocket_layout(),'rocketstar_C3_functional_layout.svg','機能配置図。寸法・タンク境界・翼面形状を定義する製造図ではない。'),
 '{{SATELLITE_LAYOUT}}':(satellite_layout(),'A-LINK_C3_functional_layout.svg','非縮尺。アンテナ・パネル・放熱面の面積と収納包絡は未確定。'),
 '{{NETWORK_LAYOUT}}':(network_layout(),'A-LINK_E3_C3_network.svg','自前衛星網と既存衛星サービスでの実証を区別した構成図。'),
 '{{MASS_CHART}}':(mass_chart(),'rocketstar_C3_mass_comparison.svg','仮定からの理想速度増分。横軸の起点は8,500 m/s。帰還成立の確認は含まない。'),
}
...
imgname='rocketstar_C3_external_concept.png'
md=md.replace('{{APPEARANCE_PAGEBREAK}}','')
md=md.replace('{{EXTERIOR_IMAGE}}',f'![rocketstar C3 外観意匠図]({imgname})')
assert '{{' not in md
(PACK/(STEM+'.md')).write_text(md)

story=[Spacer(1,8),Paragraph('rocketstar',styles['big']),Paragraph('C3 / INTEGRATED DESIGN BASELINE',styles['sub']),Spacer(1,15),Image(str(PACK/imgname),width=BW,height=BW*2/3),Spacer(1,20),Paragraph('両段再使用ロケット<br/>統合設計基準書',styles['big']),Spacer(1,10),Paragraph('外観と構成を一本化する。<br/>製造へ進むための根拠を追跡する。',styles['sub']),Spacer(1,13),Paragraph('2026.09.24  |  小型衛星輸送・A-LINK・avokado E3',styles['caption']),Paragraph('本版は外観・方式の基準案。製造仕様の凍結、実機性能の保証、飛行承認は未完了。',styles['body']),PageBreak(),Paragraph('内容',styles['h2'])]
toc=TableOfContents();toc.levelStyles=[styles['toc']];toc.dotsMinLevel=0
story.extend([toc,Spacer(1,18),Paragraph('本書はC3で選んだ構成と、未確定の製造・性能項目を区別する。詳細な根拠、依存関係、計算データは同梱パッケージを参照。',styles['caption'])])
...
while i<len(lines):
    ...
    elif line=='{{APPEARANCE_PAGEBREAK}}':
...
doc=C3Doc(pdfpath);doc.multiBuild(story)
...
for name in ['design_register.md','design_register.json','mass_study.md','mass_study.json','mass_study.py','payload_interfaces.md','payload_interfaces.json','interface_crossrefs.json','consistency_review.md','appearance_definition.json']:
...
zip_path=ROOT/'outputs/rocketstar_C3_package.zip'
