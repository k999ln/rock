...
STEM='RETURN-1_C3_Integrated_Design_Baseline'
...
class C3Doc(DesignDoc):
    def __init__(self,path):
        super().__init__(path)
        self.title='RETURN-1 C3 統合設計基準書'
    ...
    def footer(self,c,doc):
        ...
        c.drawString(LEFT,21,'RETURN-1  /  C3    |    外観・方式の基準案    |    製造・飛行 未承認')
...
assets={
 '{{ROCKET_LAYOUT}}':(rocket_layout(),'RETURN-1_C3_functional_layout.svg','機能配置図。寸法・タンク境界・翼面形状を定義する製造図ではない。'),
 ...
 '{{MASS_CHART}}':(mass_chart(),'RETURN-1_C3_mass_comparison.svg','仮定からの理想速度増分。横軸の起点は8,500 m/s。帰還成立の確認は含まない。'),
}
...
zip_path=ROOT/'outputs/RETURN-1_C3_package.zip'
