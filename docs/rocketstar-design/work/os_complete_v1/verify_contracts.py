...
legacy=Path(__file__).resolve().parents[2]/'outputs/RockstarOS_Colony_C0_1/contracts'
...
(R/'capacity_example.json').write_text(json.dumps(budget,ensure_ascii=False,indent=2)+'\n')
...
(R/'qa/contract_checks.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')
