...
状態: QEMU Developer Previewの内部限定受入済み・スマホ版はsource準備・一般公開は条件待ち（2026-09-11更新）。2026-09-09の利用者指示「CM発表に向けてOSを入れられるようにする」と、最新のスマホ書込み版の開発指示を維持する。

最新の結果と再開順は[現在の開発状態](current-state-20260911.md)。CMは最新回答で制作途中、新Siteは本人限定公開済み。Pixel 10候補のsource統合は開始したが全OS build・実機受入は未実施。初回クラウド代税別10 USD案も未承認。以前の「CM完成済み」「追加1〜2日」は[9月10日の履歴](release-followup-20260910.md)として保持し、現在の完成見積りに使わない。下記の合格条件は維持し、第3節の開始時点の未達を現在の残件へ移し替えない。
