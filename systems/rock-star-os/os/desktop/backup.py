...
from guest import BASE, state_path, running, validate_config, digest, save, directory, require, remove_stale_socket
...
def create_backup(name):
    ...
    with locked(state):
        ...
        regular(marker)
        config = json.loads(marker.read_text()); validate_config(config)
...
def restore_backup(path, new_name):
    ...
    report = json.loads((path/'backup.json').read_text())
    if report.get('schema') == 'rock-desktop-backup/2':
        return restore_stage0_backup(path, new_name, report)
    require(report.get('config', {}).get('schema') != 'rock-desktop-device/5',
...
def create_stage0_backup(name, state, config):
    ...
    require(stopped_slots(config,temporary) == update,'copied signed update state differs')
...
def restore_stage0_backup(path, new_name, report):
    ...
    import re
    config = report['config']; validate_config(config)
    require(config.get('schema') == 'rock-desktop-device/5','A/B/data backup requires explicit stage0 device')
    require(new_name != report['source_device'],'復元先には新しい端末名を指定してください。元の端末は保持します。')
    ...
    check_disk(path/'userdata.ext4')
    require(stopped_slots(config,path) == report['update'],'saved update metadata/slot set mismatch')
    state = state_path(new_name)
    with locked(state):
        require({p.name for p in state.iterdir()} <= {'lock'},'復元先に既存データがあります。新しい端末名を指定してください。')
        for disk in DISKS:
        ...
        verification = check_disk(state/'userdata.ext4')
        require(stopped_slots(config,state) == report['update'] and
