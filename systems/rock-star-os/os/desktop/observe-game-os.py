...
def main():
    ...
    parser.add_argument('--resume-stage', choices=('credit','game-a'), default='credit')
    ...
    import game_authority_ui_retention as authority_ui
    from game_exchange import sandbox
    from game_cache_retention import POLICY as GAME_READ_POLICY
    config = json.loads(args.config.read_text())
    assert config['schema'] == 'rock-desktop-device/7'
    authority_config = sandbox.load(args.sandbox_config.resolve(strict=True))
    ...
    if args.resume_from:
        ...
        if args.resume_stage == 'credit':
            assert "driver.wait('Public Game A', label='public-catalog')" in previous_report['traceback']
        else:
            assert "driver.click('‹ Gameの履歴'" in previous_report['traceback']
            assert any(s['phrase'] == '交換完了' for s in previous_report['ui_states'])
    ...
    plan = {'schema': 'rock-game-real-ui-probe/1', 'source_commit': args.commit, 'config': config,
            ...
            'scope': 'Exact candidate real OS UI/TLS; independent D0-D6 gates, no real funds, external authors or human timings',
    ...
    if previous_probe:
        plan['continuation'] = {'previous_probe': str(previous_probe), 'previous_plan_sha256': hashlib.sha256((previous_probe/'plan.json').read_bytes()).hexdigest(),
                                ...
                                'boundary': 'same running OS at '+args.resume_stage+'; already completed mutations are not repeated'}
    ...
    try:
        for cycle in (1,2):
            ...
            try:
                ...
                driver.wait(('交換完了' if args.resume_stage == 'game-a' else 'Wallet サーバーと同期済み') if cycle == 1 and previous_probe else 'ツール名・説明・IDで検索', seconds=limits['boot_seconds'], label='resume-observed-wallet' if cycle == 1 and previous_probe else 'boot')
                ...
                if cycle == 1:
                    if not previous_probe:
                        ...
                        if args.resume_stage == 'game-a':
                            driver.click('Gameの履歴', label='resume-completed-history')
                            driver.wait('合成WalletからGameへ', label='resumed-game-home')
                        else:
                            game_home(driver)
                    for game in (('B',) if previous_probe and args.resume_stage == 'game-a' else ('A','B')):
                        driver.top()
                        ...
                        driver.wait('合成WalletからGameへ', label='back-home-'+game)
                    driver.nav('hub')
                ...
                actual_authority = authority_ui.capture(authority_config, policy=authority_ui.POLICY)
                b.guest.save(folder/'authority-ui-after.json', actual_authority)
                authority.unchanged(observer.invoke('snapshot'), actual_authority['snapshot'])
                b.guest.save(folder/'authority-after.json', actual_authority['snapshot'])
                with b.closed_device(config, record) as data:
                    ...
                    b.guest.save(folder/'guest-rows.json', rows)
