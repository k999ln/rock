...
verifier checkout. The guest rechecks these installed bytes, the launcher,
OpenSSL binary and fixed public signed package. Original artifact inode/content identities are checked again even on
...
The unchanged `DeviceHub.run` verifies the installed package before creating
its worker. The tracer therefore permits exactly one preceding exec of
`/usr/bin/openssl` with the exact `pkeyutl -verify` argv generated by
`packages.verify_package`. It must be a new child of the same platform, with
the same UID/GID 1002, matching tracer and no-new-privileges state. Its three
input files must reside in the one strictly named, owned 0700 temporary
directory. Each file is opened without following links, checked for ownership,
single link, bounded size and unchanged metadata, and hashed against the exact
public text-tidy 1.0.0 DER key, canonical payload and signature. Their bytes are
never modified. The helper resumes with no delivered signal and must exit
normally before a distinct exact launcher can be faulted. A repeated helper,
unknown executable, different argv, changed input or late capture is failure.
This entire sequence shares the original four-second capture deadline. The
host requires the helper's identity, input hashes and normal exit, as well as
capture timing; old proofs without these fields cannot pass.
...
python3 -B -m unittest discover -s tests -p test_hub_signature_helper.py -v
```

The initial standalone Linux process fixture additionally validated five crashes and
...
The later source integration fixture uses the actual `Platform.Server`, owner
UID 1000 peer authentication and `DeviceHub.run`, with the unchanged compiled
C launcher and recipe interpreter in private Linux namespaces. It passed both
launcher faults, an actual owner connection closed before its reply, immutable
same-key receipt replay, conflicting-payload rejection and successful explicit
retries (four jobs, six receipts and six ordered audits). The observed deadline
was 3.008886 seconds. Eight real file/byte/ownership/deadline negative cases
were rejected. This is Linux source integration evidence, not a guest boot,
Wallet, GUI or restart acceptance result.
