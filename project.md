...
英語化した商品Siteの文字が静止配置に見えていたため、ヒーローの製品画像、ブランド名、2行コピー、補足を時間差で表示する導入へ変更した。下層の見出しは画面へ入った時に行単位で切り上がり、回転ツアーは章の切替ごとに見出し、説明、センサー詳細が再度現れる。ぼかしを短く使い、`prefers-reduced-motion`では全演出を停止して最初から読める。製品Site v30（source `7f2073c66903502fef685b8a581f501950841708`）を公開し、デスクトップの冒頭表示、英語文言、静的buildと予約API試験3件を確認した。
