...
| MAT16 | R5とavocadoMini／avokadoPro製品ラインの関係（同一・後継・別系列、価格の扱い）を決め、R5要求とMAT15の範囲を見直す | 停止中: OWNER判断待ち（2026-09-25時点、開発側で決めない）。公開Siteと台帳はMini/Pro製品ラインを正式とした一方、R5価格は未確定・E3価格非継承のまま。/preorder/は旧¥160,000／¥410,000をR5へ引き継がないと表示している。R5との関係が決まるまでMAT15の範囲とnextActionを変更しない。 | [記録](data/product-baseline.json) · [記録](docs/product-baseline.md) · [記録](docs/evidence/ledger-sync-20260925.json) |
