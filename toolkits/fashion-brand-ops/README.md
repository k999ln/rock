...
stdioではMCP clientがこのdirectoryの`.mcp.json`を読み、`initialize → tools/list → tools/call`で41個の操作をdiscover/callできます。`fashion.producer.start`は世界観と商品から市場・2週間の投稿計画・広告下書き・生成承認待ちを一括作成します。`fashion.autopilot.run`は投稿計画・下書き・承認要求など内部作業だけを最大25件まで進め、投稿・DM送信・課金などの外部作用は実行しません。HTTP modeをloopback以外へbindする場合は、bearer tokenとtenant IDの両方を必須にします。RockstarOSのplatform署名鍵、Wallet送金権限、root、任意shellはこの商品へ渡しません。
