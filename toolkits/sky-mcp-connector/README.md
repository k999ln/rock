...
Sky Tool SDK 0.1.1以降で起動したToolは、このPCの接続一覧へ自動で現れます。SDKのプロセスを起動したままSkyで「MCP → 導入・接続」を開き、「接続」を押してください。PC内の登録定義には接続用の一時キーを含むため、SDKが所有者専用の権限で保存します。終了時は定義を削除します。手動のregistry編集は不要です。
