...
const sourceGuide = 'https://github.com/k999ln/rock/blob/9abf78a80d27aa9f847c4051d20e4c552e407276/docs/preview-installation-ja.md';
const legalNotice = 'https://github.com/k999ln/rock/blob/9abf78a80d27aa9f847c4051d20e4c552e407276/docs/preview-legal-notice.md';
const releaseNotes = 'https://github.com/k999ln/rock/blob/9abf78a80d27aa9f847c4051d20e4c552e407276/docs/preview-release-notes.md';
...
export default function PreviewGuide() {
  return <main className={styles.page}>
    ...
    <article className={styles.guide}>
      ...
      <div className={styles.notice}><strong>QEMU・合成環境で内部検証済みです。</strong><p>これは現在の操作手順です。製品ライセンスは未確定で、一般公開の配布は未開始です。公開条件を確認した後に案内する配布記録で、ダウンロードURL、版とファイルのハッシュを確認してください。</p></div>
      ...
      <section id="wallet"><h2>04 / Walletの状態とGameを確認する</h2><p>Walletでは合成残高、保留、請求、入金の状態を区別します。実費・実収益に接続していないものは、未接続と表示します。月額は同一契約につき888 USD centsで、同じ契約の複数端末で重複しない構成です。月額の同意は本人が別に選びます。この版の請求は合成残高を使う試験で、実課金には接続していません。</p><p>同じ配布候補で、合成Game A/Bの接続・交換・保存履歴を内部検証しました。公開試験の登録、試験認証、Wallet利用条件を確認した後、明示操作で試験残高を一度追加する構成です。Gameへの接続と、今回の交換条件への承認は別々です。購入額、ゲーム手数料、外部実費、保留合計、受取り単位を確認します。</p><p>照合中に新しい交換を作り直さず、元の履歴から同じ要求を確認します。Gameが使えない間もHubの保存結果と正常終了へ戻れるようにしています。</p><p>試験設定のGame接続は1時間有効です。期限切れ・失効後の新しい接続はこの版では未対応です。画面とSDKに理由を表示し、元の要求の照合と完了済み購入の履歴は保持します。</p></section>
