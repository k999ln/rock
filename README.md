...
WalletをOS内外の価値を正規化するValue Routerとして扱い、外部サービスへの支出を必ず提案・リスク判定・承認・隔離署名・adapter実行・receipt照合へ通す[Value/Spend Runtimeのhost vertical slice](docs/value-spend-runtime.md)を追加しました。Sky（内部互換名Hub）とMCPは同一の入口です。第一号はPolymarket dry-run adapterで、SIMULATION/PAPERのみ利用でき、LIVE・実資金・外部署名は明示許可まで無効です。
...
**[設計v1.1](docs/os-sky-wallet-game-design.md)の実装は承認済みです。** [承認範囲](docs/execution-approval-20260909.md)に従い、専用branchでnativeと設計を統合しています。RQ20で市場連携の共通runtimeとdry-runが追加承認されましたが、公開・実機・MetaMask実資金・Polymarket LIVEは条件付き了承のままです。達成演出は見送ります。
...
| FB01 | Instagram運用・受注型ブランド管理をRockstarOS Sky商品とMCPへ統合 | 完了 | [記録](docs/fashion-brand-ops-integration.md) |
...
| VS02 | Skyと同じ操作言語でValue/Spendの安全な支出画面を実装 | 完了 | [記録](components/value-spend-panel.tsx) · [記録](components/operations-workspace.tsx) · [記録](tests/value-spend-ui.test.mjs) · [記録](docs/current-state-20260911.md) |
