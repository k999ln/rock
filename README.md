...
Web版はデスクトップで左のBot・履歴欄を常時表示し、左3：右7の幅で会話を進めます。Botを切り替えても新しい会話を始めても左欄は残ります。狭い画面では左欄を開閉できます。Bot一覧は最初に4件と選択中のBotを表示します。残りは「すべてのBotを見る」か検索から選べます。Toolへの接続と実行には本人のサインインや必要な承認が残ります。
Zemaを直接開くと担当を自動選択する新しい会話から始まります。Botを選んだ後も「新しい会話」で入力と表示をリセットでき、依頼例を押すと送信前に入力欄へ入ります。担当Botは会話上部に表示し、依頼文は対応するToolの入力欄へ引き継ぎます。ローカル会話モデルが未接続でもToolの入力・実行は利用できますが、自然なAI返答にはモデルの接続が必要です。
