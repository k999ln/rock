...
| Android物理端末 | Pixel 10 GL066固定、署名source／partition構成、初回flash方針、7項目のproduction構成           | 6必須gate中1合格                       | UI/Broker分離、BSP、full build、SELinux、CTS/VTS、OTA、純正復旧    |
...
- **Android production構成**: `dev.rock.automation`をheadless Platform Brokerとして維持し、Home／Sky／Zemaを`dev.rock.shell`へ分離します。Local AI、Tool、MCP、Provider、Operator Agentは別UID／別SELinux domain、Operator Dockは利用者OS外です。7項目の正本と現在の未実装境界は[Android production architecture](docs/android-production-architecture.md)に固定しています。
