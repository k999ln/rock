...
Web版のBot一覧は最初に4件と選択中のBotを表示します。残りは「すべてのBotを見る」か検索から選べます。Toolへの接続と実行には本人のサインインや必要な承認が残ります。
Zemaを直接開くと担当を自動選択する新しい会話から始まります。Botを選んだ後も「新しい会話」で入力と表示をリセットでき、依頼例を押すと送信前に入力欄へ入ります。
