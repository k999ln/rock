...
次の作業: Pixelを再接続し、実行中のnative Sky選択仕事をseedした後に端末を実再起動して、lease回収、2 Tool、結果、7履歴eventまでrecover phaseで確認する。backup v2のsource／UI／emulator受入は完了したため、純正復旧artifactを確保してから物理wipe復元を行う。並行してvendor inventory、production署名、Operator Agentの事前gateを進める。
<!-- project-status:end -->
