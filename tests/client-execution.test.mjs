...
await test('result report retries reuse the same payload and never run the tool twice', async () => {
  const reports = [];
  let executions = 0;
...
await test('ambiguous start stops execution and a later explicit run is allowed', async () => {
  let executions = 0;
  globalThis.fetch = async (path, init) =>
    path === '/api/jobs'
...
await test('completion outage retains output with a warning and does not fabricate a failed result', async () => {
  const statuses = [];
  globalThis.fetch = async (path, init) => {
    const body = JSON.parse(init.body);
...
await test('input and output content stay off the management API, and the tab admits one task', async () => {
  const bodies = [];
  let release;
  let started;
