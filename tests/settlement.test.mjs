...
void test('fee cannot exceed low income and no remainder is carried', () => {
  const actual = estimate({ ...defaultEstimate, revenue: 500 });
  assert.equal(actual.recoveredCost, 74);
  assert.equal(actual.fee, 426);
  assert.equal(actual.payout, 0);
  assert.equal(actual.net, 0);
});
void test('monthly fee capped at 8.88 USD, all operating costs included', () => {
...
void test('zero-cost run and fractional FX round in whole JPY', () => {
  const actual = estimate({
    ...defaultEstimate,
    revenue: 2000,
    fx: 155.55,
