...
void test('product baseline rejects lost requirements, stale-as-live claims and mismatched source evidence', () => {
  ...
  assert.throws(() => validateBaseline(unapprovedMarket), /Markets/);
  ...
  unauthorizedRuntime.marketExploration.orderExecutionEnabled = true;
  assert.throws(() => validateBaseline(unauthorizedRuntime), /注文/);
  const fakeMarketRevenue = structuredClone(source);
  fakeMarketRevenue.marketExploration.countsAsFundRevenue = true;
  assert.throws(() => validateBaseline(fakeMarketRevenue), /未確定収益/);
  const unsafeBot = structuredClone(source);
  unsafeBot.marketExploration.strategyLab.liveModeToggleEnabled = true;
  assert.throws(() => validateBaseline(unsafeBot), /注文/);
