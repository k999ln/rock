...
test('job lifecycle records completion and history exactly once; terminal state is immutable', async () => {
...
test('ownership is checked for every job, device, control and book operation', async () => {
  const { a, b } = fixture(),
    input = job(),
...
test('active slot, hourly limit and disabled tools are enforced before dispatch', async () => {
...
test('disabled-after-queue blocks start and timeout frees the slot without re-executing', async () => {
...
test('timeout rejects late completion, and active work cannot be claimed as cancelled', async () => {
  const { a, advance } = fixture(),
    input = job();
  await a.createJob(input);
  await a.changeJob(input.id, { action: 'start' });
...
test('MCP dispatch requires recent owned device; revocation and stale connections block new work', async () => {
  const { a, advance } = fixture(),
...
test('job and accounting validation reject forged fields, invalid dates and unsafe values', async () => {
  const { a } = fixture();
  for (const change of [
    { tool: 'shell' },
    { id: '1' },
...
test('device revocation after queuing prevents the start claim', async () => {
  const { a } = fixture(),
    deviceId = crypto.randomUUID();
  await a.device({ id: deviceId, name: 'PC', action: 'connect' });
...
test('book originals are immutable, retries are idempotent, reversals conserve totals and cannot be reversed twice', async () => {
  const { a } = fixture(),
    input = {
      id: crypto.randomUUID(),
      kind: 'revenue',
...
test('history failure rolls back completion so a report can safely be retried', async () => {
  const { a, sqlite } = fixture(),
    input = job();
  await a.createJob(input);
  await a.changeJob(input.id, { action: 'start' });
