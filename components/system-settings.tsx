        ...
        <div className={styles.content}>
          ...
          <section className={styles.health} aria-label="システム状態">
            <HealthItem
              icon={<Wifi />}
              label="ブラウザ接続"
              value={online ? 'オンライン' : 'オフライン'}
              state={online ? 'ok' : 'warn'}
            />
            <HealthItem
              icon={<Cable />}
              label="PC Connector"
              value={connector ? '接続済み' : '未接続'}
              state={connector ? 'ok' : 'idle'}
            />
            <HealthItem
              icon={<UserRound />}
              label="利用者"
              value={accountLabel}
              state={account === 'signed-in' ? 'ok' : 'idle'}
            />
          </section>
          ...
          <SettingsGroup title="自分のOS">
            ...
            <SettingButton
              ...
              title="ホーム画面に追加"
              detail={
                standalone ? 'アプリとして起動中' : 'iPhone / PCへ追加する手順'
              }
          ...
          <SettingsGroup title="接続とアプリ">
            <SettingButton
              ...
              tone="green"
              title="このPCとMCP"
              detail={connector ? 'Connector接続済み' : '接続アプリを準備する'}
          ...
          <SettingsGroup title="アカウントとデータ">
            {account === 'signed-out' ? (
              <SettingAnchor
                href="/signin-with-chatgpt?return_to=%2Fsettings"
                icon={<UserRound />}
                tone="orange"
                title="サインイン"
                detail="自分の実行履歴を開く"
          ...
          <SettingsGroup title="更新と復旧">
            ...
            />
            <SettingLink
              href="/rockstaros/guide#recovery"
              icon={<Smartphone />}
              tone="gray"
              title="OS導入・バックアップ・復旧"
              detail="Developer Previewの手順と制限"
            />
          </SettingsGroup>
...
function IconBox({ tone, children }: { tone: string; children: ReactNode }) {
  return (
    <span className={`${styles.itemIcon} ${styles[tone]}`}>{children}</span>
  );
}
...
function SettingAnchor({
  ...
  return (
    <a className={styles.item} href={href} target="_top">
      <IconBox tone={tone}>{icon}</IconBox>
      <SettingCopy title={title} detail={detail} />
      <ArrowRight size={17} />
    </a>
...
function SettingRow({ icon, tone, title, detail }: SettingProps) {
  return (
    <div className={styles.item}>
      <IconBox tone={tone}>{icon}</IconBox>
      <SettingCopy title={title} detail={detail} />
      <CheckCircle2 size={17} className={styles.rowCheck} />
    </div>
  );
}
